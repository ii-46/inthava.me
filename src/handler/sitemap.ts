...
import { getAllPublicWorkshops, getPublicWorkshops } from "../service/workshop";

export async function sendSitemap(req: Request, res: Response) {
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  try {
    ...
    sitemaps[0] = sitemaps[0].replace(/date/g, new Date(allArticle[0].updatedAt).toISOString());
    sitemaps[1] = sitemaps[1].replace("date", new Date(allArticle[0].updatedAt).toISOString());
    sitemaps.push(...articleSitemap);
    const allWorkshop = await getAllPublicWorkshops();
    const workshopSitemap = [];
    if (allWorkshop.length > 0) {
      sitemaps[2] = sitemaps[2].replace("date", new Date(workshopSitemap[0].updatedAt).toISOString());
      workshopSitemap.push(...(allWorkshop.map((workshop) => {
        return `<url>
